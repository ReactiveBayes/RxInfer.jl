...
- Compact, single-line `Base.show` methods for every Tier A callback event (`Before/AfterModelCreationEvent`, `Before/AfterInferenceEvent`, `Before/AfterIterationEvent`, `Before/AfterDataUpdateEvent`, `OnMarginalUpdateEvent`, `Before/AfterAutostartEvent`) plus a `_show_span` helper that truncates UUID4 span identifiers to a 4-character prefix, so trace breadcrumbs in TBLogger's Text tab no longer dump raw struct contents ([#638](https://github.com/ReactiveBayes/RxInfer.jl/issues/638)). Pairs with the matching ReactiveMP-side `Base.show` work for `MessageMapping`, `MessageProductContext`, `AnnotationDict`, `FormConstraintCheck*`, and the Tier B events.

### Changed
- `TensorBoardLoggerExt` per-event text breadcrumbs now delegate to each event's `Base.show` via `repr(ev)` instead of building bespoke `"k1: v1 | k2: v2 | …"` strings (or calling the now-removed `_format_fields` helper). Roughly 110 lines of inline formatting collapse into one-liners and Tier B/C struct dumps disappear from TBLogger Text output once paired with the ReactiveMP show methods. ([#638](https://github.com/ReactiveBayes/RxInfer.jl/issues/638))
