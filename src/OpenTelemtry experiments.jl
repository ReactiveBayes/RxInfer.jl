...
function find_spans(trace_events::Vector{TracedEvent})
    ...
    spans = [(starts[k], stops[k]) for k in collect(keys(starts)) ∩ keys(stops)]
...
function to_observability_string(
    z::Vector{<:ReactiveMP.AbstractMessage}, keyname::Symbol
)
...
to_observability_string(z::Union{ReactiveMP.MessageProductContext, ReactiveMP.MessageMapping, RxInfer.ProbabilisticModel}, keyname::Symbol) = "<omitted>"
        ...
        Expr(
            ...
            (
                Expr(
                    ...
                    f,
                    :(to_observability_string(
                        getfield(x, $(QuoteNode(f))), $(QuoteNode(f))
                    )),
...
to_otel_value(v::Vector) = Dict(
    "arrayValue" => Dict("values" => map(to_otel_value, v))
)

# ╔═╡ 0ee8645e-3c97-4a0b-8d2f-471f8b32ba0d
to_otel_dict(d::AbstractDict) = [
    Dict("key" => k, "value" => to_otel_value(v)) for (k, v) in d
]

# ╔═╡ c77e4434-df63-4aed-a961-254cc5c59c0c
to_otel_value(v::Dict) = Dict(
    "kvlistValue" => Dict("values" => to_otel_dict(v))
)
...
@generated function event_phase(t::ReactiveMP.Event)
    name = t.name.name
    if startswith(string(name), "Before")
        "B"
    elseif startswith(string(name), "After")
        "E"
    else
        "X"
    end
...
function to_perfetto(te::TracedEvent; time_delta::Int64 = 0)
    ...
    (
        ...
        ph = ph,
        ts = Float64(
            Int64(te.time_ns) + time_delta + (ph == "E" ? 1 : 0), # add 1 ns to End events because some recorded events are 0ns long, which is otherwise not supported in perfetto.
        ) / 1000,
...
perfetto = Dict(
    ...
    "metadata" => Dict(
        "clock-domain" => "MONO",
        "trace-capture-datetime" => "command_line" => "RxInferBoard",
        "higres-ticks" => true,
    ),
)
...
function perfetto_view(
    perfetto_json_contents; name = "$(Time(now())) RxInfer trace"
)
