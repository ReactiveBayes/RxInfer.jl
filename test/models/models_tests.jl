...
@testitem "A `Distribution` object as priors in arguments" begin
        ...
        for seed in (123, 456), n in (50, 100)
            ...
            for ts in testsets
                result_with_prior_as_input = infer(
                    model = beta_bernoulli_priors(prior = ts[:prior]),
                    returnvars = KeepLast(),
                    data = (y = data,),
                    iterations = 10,
                    free_energy = true
    ...
    @testset "IID Gaussian model" begin
        for seed in (123, 456),
            n in (50, 100),
            constraints in (MeanField(), @constraints(
                begin
                    q(μ, τ) = q(μ)q(τ)
                end
            ))
            ...
            for ts in testsets
                ...
                @test result_with_prior_as_input.posteriors[:μ] ==
                    result_with_params_as_input.posteriors[:μ]
                @test result_with_prior_as_input.posteriors[:τ] ==
...
@testitem "Unknown object in the model specification should throw a user-friendly error" begin
    ...
    @test_throws "`$(SomeArbitraryDistribution)` cannot be used as a factor node" infer(
        model = a_model_with_unknown_distribution(), data = (y = ones(3),)
    ).posteriors[:θ]
end
...
@testitem "Data variables should fold automatically" begin
    ...
    @testset for modelfn in [
        sum_datavars_as_gaussian_mean_1, sum_datavars_as_gaussian_mean_2
    ]
        result = infer(
            model = modelfn(),
    ...
    @testset for modelfn in [
        sum_datavars_as_gaussian_mean_3, sum_datavars_as_gaussian_mean_4
    ]
        result = infer(
            model = modelfn(),
    ...
    @model function idx_datavars_as_gaussian_mean(y, a, b)
        x ~ Normal(
            mean = dot(getindex(a, 1:2), getindex(b, 1:2, 1)), variance = 1.0
        )
        y ~ Normal(mean = x, variance = 1.0)
    end
