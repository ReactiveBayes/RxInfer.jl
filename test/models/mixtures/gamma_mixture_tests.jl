@testitem "Gamma mixture model" begin
    ...
    @model function gamma_mixture_model(
        y, nmixtures, priors_as, priors_bs, prior_s
    )
        ...
        for i in 1:nmixtures
            as[i] ~
            Gamma(shape = shape(priors_as[i]), rate = rate(priors_as[i]))
            bs[i] ~
            Gamma(shape = shape(priors_bs[i]), rate = rate(priors_bs[i]))
        end
    ...
    @test_plot "models" "gamma_mixture" begin
        ...
        p1 = histogram(
            ...
            normalize = :pdf,
            label = "data",
            title = "Generated mixtures",
            opacity = 0.3
        )
        p1 = plot!(
            range(0.0, 1.0, length = 100),
            (x) -> mixing[1] * pdf(mixtures[1], x),
            label = "component 1",
            linewidth = 3.0
        )
        p1 = plot!(
            range(0.0, 1.0, length = 100),
            (x) -> mixing[2] * pdf(mixtures[2], x),
            label = "component 2",
            linewidth = 3.0
        )
        ...
        p2 = histogram(
            ...
            normalize = :pdf,
            label = "data",
            title = "Inferred mixtures",
            opacity = 0.3
        )
        p2 = plot!(
            range(0.0, 1.0, length = 100),
            (x) -> _mixing[1] * pdf(_dists[1], x),
            label = "component 1",
            linewidth = 3.0
        )
        p2 = plot!(
            range(0.0, 1.0, length = 100),
            (x) -> _mixing[2] * pdf(_dists[2], x),
            label = "component 2",
            linewidth = 3.0
        )
