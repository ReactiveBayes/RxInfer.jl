...
@testitem "Model mixture" begin
    ...
    @testset "Check inference results" begin
        ...
        @test getdata(result1.posteriors[:θ]) ==
            getdata(resultswitch.posteriors[:in1])
        @test getdata(result2.posteriors[:θ]) ==
            getdata(resultswitch.posteriors[:in2])
        @test getdata(resultswitch.posteriors[:in1]) ==
            component(getdata(resultswitch.posteriors[:θ]), 1)
        @test getdata(resultswitch.posteriors[:in2]) ==
            component(getdata(resultswitch.posteriors[:θ]), 2)
        @test getdata(resultswitch.posteriors[:selector]).p ≈
