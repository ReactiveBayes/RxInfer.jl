@testitem "Beta-Bernoulli model" begin
    ...
    @test all(
        v -> v ≈ 2828.0533343622483, result_with_no_iterations.free_energy
    )
    ...
    for p in (0.75, 0.5, 0.25), n in (5_000, 10_000)
        local data_for_specific_p = float.(
            rand(StableRNG(123), Bernoulli(p), n)
        )
        ...
        streaming_subscription = subscribe!(
            result_for_streaming_version.posteriors[:θ],
            (new_posterior) -> begin
                ...
                streaming_analytical_a[] += isone(
                    streaming_analytical_observation
                )
                streaming_analytical_b[] += iszero(
                    streaming_analytical_observation
                )
                ...
                if new_posterior_a !== streaming_analytical_a[] ||
                    new_posterior_b !== streaming_analytical_b[]
        ...
        @test result_for_specific_p.posteriors[:θ] ==
            result_for_streaming_version.history[:θ][1]
    ...
    for θinit in
        (Beta(1.0, 1.0), Beta(2.0, 2.0), Beta(2.0, 7.0), Beta(7.0, 2.0))
    ...
    for T in (BigFloat, Float64, Float32)
        result_with_specific_free_energy_type = infer(
            model = beta_bernoulli(a = 2.0, b = 7.0),
            data = (y = dataset,),
            iterations = 10,
            free_energy = T
