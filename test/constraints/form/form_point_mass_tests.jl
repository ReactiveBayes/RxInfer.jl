@testitem "PointMassFormConstraint" begin
    ...
    const arbitrary_dist_1 = ContinuousUnivariateLogPdf(
        RealLine(), (x) -> logpdf(NormalMeanVariance(0, 1), x)
    )
    const arbitrary_dist_2 = ContinuousUnivariateLogPdf(
        HalfLine(), (x) -> logpdf(Gamma(1, 1), x)
    )
    const arbitrary_dist_3 = ContinuousUnivariateLogPdf(
        RealLine(), (x) -> logpdf(NormalMeanVariance(-10, 10), x)
    )
    const arbitrary_dist_4 = ContinuousUnivariateLogPdf(
        HalfLine(), (x) -> logpdf(GammaShapeRate(100, 10), x)
    )
    const arbitrary_dist_5 = ContinuousUnivariateLogPdf(
        HalfLine(), (x) -> logpdf(GammaShapeRate(100, 100), x)
    )
    ...
    @testset "starting point" begin
        ...
        @test_throws ErrorException call_starting_point(
            constraint, arbitrary_dist_2
        )
    ...
    @testset "optimizer" begin
        ...
        @test isapprox(
            mean(constrain_form(constraint, arbitrary_dist_1)), 0.0, atol = 0.1
        )
        @test isapprox(
        ...
        gopt_constraint = PointMassFormConstraint(
            starting_point = (args...) -> [1.0]
        )
    ...
    @testset "fast path for Distribution" begin
        ...
        for mode in randn(4)
            @test mean(
                constrain_form(constraint, MyDistributionWithMode(mode))
            ) === mode
        end
    end
    ...
    @testset "optimizer for generic f" begin
        ...
        for _ in 1:3
            ...
            f = ContinuousUnivariateLogPdf(
                DomainSets.HalfLine(), (x) -> logpdf(d1, x) + logpdf(d2, x)
            )
