@testitem "CompiledRunner: conjugate posteriors, constraints, and free energy" begin
    ...
    function compare(
        ...
        for workers in unique((1, Threads.nthreads()))
            actual = infer(;
                args..., options = (runner = CompiledRunner(; workers),)
            )
            ...
            for name in keys(baseline.posteriors)
                @test mean(actual.posteriors[name]) ≈
                    mean(baseline.posteriors[name]) rtol=1e-4 atol=1e-6
                @test var(actual.posteriors[name]) ≈
                    var(baseline.posteriors[name]) rtol=1e-4 atol=1e-6
            end
    ...
    @test length(parallel.model.metadata[:inference_runner].operations) <
        10 * 2048
...
@testitem "CompiledRunner: structured hidden Markov marginals" begin
    ...
    for name in (:A, :B, :s0), statistic in (mean, var)
        @test statistic(actual.posteriors[name]) ≈
            statistic(baseline.posteriors[name]) rtol=1e-4 atol=1e-6
    end
    for statistic in (mean, var)
        @test statistic.(actual.posteriors[:s]) ≈
            statistic.(baseline.posteriors[:s]) rtol=1e-4 atol=1e-6
    end
...
@testitem "CompiledRunner: required self-messages and explicit rejection" begin
    ...
    for policy in (
        nothing,
        RequireMessageFunctionalDependencies(in = NormalMeanVariance(0.0, 1.0)),
    )
        args = (;
            model = compiled_probit(dependencies = policy),
            data = (y = [false, false, true, true, true],),
            iterations = 100,
    ...
    @test_throws UnsupportedCompiledFeature infer(
        model = compiled_stream_policy(),
        data = (y = 0.5,),
        options = (runner = CompiledRunner(),),
        session = nothing,
    )
end
...
@testitem "CompiledRunner: gamma mixture and point-mass form constraints" begin
    ...
    args = (;
        model = compiled_gamma_mixture(),
        data = (y = [0.4, 0.7, 1.1, 1.8, 2.2, 2.8],),
        constraints,
        initialization = init,
        iterations = 200,
        returnvars = KeepLast(),
    ...
    actual = infer(; args..., options = (runner = CompiledRunner(),))
    @test any(
        kernel -> kernel isa ReactiveMP.CompiledVariationalProductKernel,
...
@testitem "CompiledRunner: Mixture log-scale annotations" begin
    ...
    args = (;
        model = compiled_beta_mixture(),
        data = (y = [1.0, 1.0, 0.0, 1.0],),
...
@testitem "CompiledRunner: streaming autoupdates, bounded history, and restart" begin
    ...
    args = (;
        model = compiled_online(),
        data = (y = [1.0, 0.0, 1.0, 1.0],),
        autoupdates = updates,
        initialization = init,
        iterations = 2,
        keephistory = 4,
        returnvars = [:p],
        historyvars = (p = KeepLast(),),
        free_energy = true,
        session = nothing,
    )
    baseline = infer(; args...)
...
@testitem "CompiledRunner: message autoupdates, variable iterations, and failures" begin
    ...
    iterations = Ref(2)
    engine = infer(
        model = compiled_online_message(m = 0.0, v = 1.0),
        datastream = source,
    ...
    count = Ref(0)
    callbacks = (
        on_marginal_update = event -> begin
            count[] += 1
    ...
    failure = infer(;
        args...,
        callbacks = (
            after_iteration = event -> error("injected iteration failure"),
        ),
...
@testitem "CompiledRunner: nonlinear Delta and observed arguments" begin
    ...
    for fn in (delta_affine, delta_smooth),
        method in (Linearization(), Unscented())

        args = (;
            model = compiled_delta(; fn, method),
            data = (y = 0.7, a = 0.2),
            initialization = init,
            iterations = 30,
            returnvars = KeepLast(),
...
@testitem "CompiledRunner: seeded nonlinear CVI compatibility" begin
    ...
    for seed in (11, 22, 33)
        ...
        for name in (:x, :z)
            @test mean(actual.posteriors[name]) ≈
                mean(reference.posteriors[name]) atol=0.08 rtol=0
            @test var(actual.posteriors[name]) ≈ var(reference.posteriors[name]) atol=0.03 rtol=0
        end
        push!(
            differences,
            mean(actual.posteriors[:x]) - mean(reference.posteriors[:x]),
        )
...
@testitem "CompiledRunner: missing data predictions and snapshots" begin
    ...
    args = (;
        model = compiled_predict(),
        data = (y = [1.0, missing, -0.5],),
        iterations = 3,
    ...
    @test mean.(actual.predictions[:y][end]) ≈
        mean.(baseline.predictions[:y][end])
    @test var.(actual.predictions[:y][end]) ≈
        var.(baseline.predictions[:y][end])
    shorthand = infer(
        model = compiled_predict(),
        data = (y = [1.0, missing, -0.5],),
    ...
    @test mean.(shorthand.predictions[:y]) ≈
        mean.(baseline.predictions[:y][end])
end
