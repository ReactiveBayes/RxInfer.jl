@testitem "Autoupdates specs" begin
    ...
    @testset "Single variables on LHS, single variable on RHS" begin
        ...
        @test getmapping(autoupdate1) ===
            AutoUpdateMapping(f, (AutoUpdateFetchMarginalArgument(:θ),))

        autoupdate2 = getautoupdate(autoupdates, 2)
        @test getvarlabels(autoupdate2) === AutoUpdateVariableLabel(:b)
        @test getmapping(autoupdate2) ===
            AutoUpdateMapping(g, (AutoUpdateFetchMessageArgument(:θ),))
    end
    ...
    @testset "Multiple variables on LHS, single variable on RHS" begin
        ...
        autoupdate1 = getautoupdate(autoupdates, 1)
        @test getvarlabels(autoupdate1) ===
            (AutoUpdateVariableLabel(:a), AutoUpdateVariableLabel(:b))
        @test getmapping(autoupdate1) ===
            AutoUpdateMapping(params, (AutoUpdateFetchMarginalArgument(:θ),))
    end
    ...
    @testset "Single variables on LHS, complex expression on RHS" begin
        ...
        autoupdate1 = getautoupdate(autoupdates, 1)
        @test getvarlabels(autoupdate1) === AutoUpdateVariableLabel(:a)
        @test getmapping(autoupdate1) === AutoUpdateMapping(
            getindex,
            (
                AutoUpdateMapping(
        ...
        autoupdate2 = getautoupdate(autoupdates, 2)
        @test getvarlabels(autoupdate2) === AutoUpdateVariableLabel(:b)
        @test getmapping(autoupdate2) === AutoUpdateMapping(
            getindex,
            (
                AutoUpdateMapping(
                    params, (AutoUpdateFetchMarginalArgument(:θ),)
    ...
    @testset "`@autoupdates` can accept a function" begin
        ...
        @autoupdates function myautoupdates(
            unused, input::S; keyword = input
        ) where {S <: String}
            z = mean(q(z)) + keyword
        end
        ...
        @test getmapping(autoupdate1_true) ===
            AutoUpdateMapping(mean, (AutoUpdateFetchMarginalArgument(:x),))
        ...
        @test getmapping(autoupdate1_false) ===
            AutoUpdateMapping(mean, (AutoUpdateFetchMarginalArgument(:y),))
        ...
        @test getmapping(autoupdate1_string) === AutoUpdateMapping(
            +,
            (
                AutoUpdateMapping(mean, (AutoUpdateFetchMarginalArgument(:z),)),
                "hello",
...
@testitem "Empty autoupdates are not allowed" begin
    @test_throws "`@autoupdates` did not find any auto-updates specifications. Check the documentation for more information." eval(
        :(@autoupdates begin end)
    )
end

@testitem "`@autoupdates` requires a block of code" begin
    @test_throws "Autoupdates requires a block of code `begin ... end` or a full function definition as an input" eval(
        :(@autoupdates 1 + 1)
    )
    @test_throws "Autoupdates requires a block of code `begin ... end` or a full function definition as an input" eval(
        :(@autoupdates a = q(θ))
...
@testitem "The `autoupdates` macro should support different options" begin
    ...
    @test_throws "Unknown option for `@autoupdates`: hello = false. Supported options are [ `warn`, `strict` ]" eval(
        :(@autoupdates [hello = false] function hello_function()
            a, b = params(q(θ))
        end)
    )
    ...
    @test_throws "Invalid value for `warn` option. Expected `true` or `false`." eval(
        :(@autoupdates [warn = "hello"] function hello_function()
            a, b = params(q(θ))
        end)
    )
    ...
    @test_throws "Invalid value for `strict` option. Expected `true` or `false`." eval(
        :(@autoupdates [strict = "hello"] function hello_function()
            a, b = params(q(θ))
        end)
    )
end
...
@testitem "The `autoupdates` structure can be prepared for a specific model #1 - Beta Bernoulli" begin
    ...
    @testset "Warning/error if the varlabel in `autoupdates` have been reserved with constants" begin
        ...
        @test_logs check_model_generator_compatibility(
            autoupdates_nowarn, model
        )
    ...
    @testset "Create deferred data handlers from the `@autoupdates` specification" begin
        ...
        @test @inferred(
            autoupdates_data_handlers(autoupdates_with_indices_1)
        ) === (ins = DeferredDataHandler(),)
        @test @inferred(
            autoupdates_data_handlers(autoupdates_with_indices_2)
        ) === (
            ins = DeferredDataHandler(),
    ...
    @testset "Check that variables have been fetched correctly" begin
        ...
        for autoupdate in autoupdates_to_test
            ...
            autoupdates_for_model = prepare_autoupdates_for_model(
                autoupdate, model
            )
            ...
            @test ReactiveMP.getdata.(updates_for_a) ==
                [PointMass(1), PointMass(2.0)]
            @test ReactiveMP.getdata.(updates_for_b) ==
            ...
            @test ReactiveMP.getdata.(updates_for_b) ==
                [PointMass(2), PointMass(2.0)]
            @test ReactiveMP.getdata.(updates_for_y) ==
                [PointMass(1), PointMass(0)]
...
@testitem "The `autoupdates` structure can be prepared for a specific model #2 - Beta Bernoulli" begin
    ...
    for autoupdate in
        (autoupdates_1, autoupdates_2, autoupdates_3, autoupdates_4)
        extra_data_handlers = autoupdates_data_handlers(autoupdate)
        data_handlers = (y = DeferredDataHandler(), extra_data_handlers...)
        options = convert(ReactiveMPInferenceOptions, (;))
        plugins = PluginsCollection(
            VariationalConstraintsPlugin(), ReactiveMPInferencePlugin(options)
        )
        model = create_model(
            with_plugins(beta_bernoulli_vector_based(), plugins) | data_handlers
        )
        variable_a = getvariable(getindex(getvardict(model), :a))
        variable_b = getvariable(getindex(getvardict(model), :b))
        variable_y = getvariable(getindex(getvardict(model), :y))
...
@testitem "The `autoupdates` structure can be prepared for a specific model #3 - Beta Bernoulli" begin
    ...
    for autoupdate in (
        ...
        @test length(updates_for_y) === 1
        @test ReactiveMP.getdata.(marginals_θ) == [Beta(2.0, 2.0)]
        @test ReactiveMP.getdata.(updates_for_ins) ==
...
@testitem "`fetch` for `AutoUpdateMapping`" begin
    ...
    @test @inferred(
        fetch(AutoUpdateMapping(+, (1, AutoUpdateMapping(+, (1, 1)))))
    ) === 3
    @test @inferred(
        fetch(
            AutoUpdateMapping(Base.Broadcast.BroadcastFunction(+), ([1], [2]))
        )
    ) == [3]
    ...
    @test @inferred(fetch(AutoUpdateMapping(+, (stream_of_1, stream_of_2)))) ===
        3
    @test @inferred(
        fetch(
            AutoUpdateMapping(
    ...
    @test_throws "The initial value for `x` has not been specified, but is required in the `@autoupdates`." fetch(
        AutoUpdateMapping(+, (empty_stream_for_x, 1))
    )
    @test_throws "The initial value for `y` has not been specified, but is required in the `@autoupdates`." fetch(
        AutoUpdateMapping(+, (1, empty_stream_for_y))
    )
...
@testitem "autoupdate_convert_mapping_expr" begin
    import RxInfer:
        autoupdate_convert_mapping_expr,
        AutoUpdateMapping,
        AutoUpdateFetchMarginalArgument,
        AutoUpdateFetchMessageArgument
    ...
    @test autoupdate_convert_mapping_expr(:(f(q(x[1, 1])))) ==
        :(RxInfer.AutoUpdateMapping(
        f, (RxInfer.AutoUpdateFetchMarginalArgument(:x, (1, 1)),)
    ))
    @test autoupdate_convert_mapping_expr(:(g(μ(x)))) ==
        :(RxInfer.AutoUpdateMapping(
        g, (RxInfer.AutoUpdateFetchMessageArgument(:x),)
    ))
    @test autoupdate_convert_mapping_expr(:(g(μ(x[1])))) ==
        :(RxInfer.AutoUpdateMapping(
        g, (RxInfer.AutoUpdateFetchMessageArgument(:x, (1,)),)
    ))
    @test autoupdate_convert_mapping_expr(:(g(μ(x[1, 1])))) ==
        :(RxInfer.AutoUpdateMapping(
        g, (RxInfer.AutoUpdateFetchMessageArgument(:x, (1, 1)),)
    ))
    ...
    @test autoupdate_convert_mapping_expr(:(f(q(x), 2))) ==
        :(RxInfer.AutoUpdateMapping(
        f, (RxInfer.AutoUpdateFetchMarginalArgument(:x), 2)
    ))
    @test autoupdate_convert_mapping_expr(:(f(q(x), g(3, 2)))) ==
        :(RxInfer.AutoUpdateMapping(
        f, (RxInfer.AutoUpdateFetchMarginalArgument(:x), g(3, 2))
    ))
    # hierarchial call
    @test autoupdate_convert_mapping_expr(:(f(f(q(x))))) ==
        :(RxInfer.AutoUpdateMapping(
        f,
        (
            RxInfer.AutoUpdateMapping(
...
@testitem "autoupdate_convert_labels_expr" begin
    ...
    @test autoupdate_convert_labels_expr(:a) ==
        :(RxInfer.AutoUpdateVariableLabel(:a))
    @test autoupdate_convert_labels_expr(:(a[1])) ==
        :(RxInfer.AutoUpdateVariableLabel(:a, (1,)))
    @test autoupdate_convert_labels_expr(:(a[1, 1])) ==
        :(RxInfer.AutoUpdateVariableLabel(:a, (1, 1)))
    @test autoupdate_convert_labels_expr(:(a, b)) == :((
        RxInfer.AutoUpdateVariableLabel(:a), RxInfer.AutoUpdateVariableLabel(:b)
    ))
    ...
    @test_throws "Cannot create variable label from expression `f(a)`" autoupdate_convert_labels_expr(
        :(f(a))
    )
end
...
@testitem "autoupdate_parse_autoupdate_specification_expr" begin
    ...
    @test autoupdate_parse_autoupdate_specification_expr(
        :spec, :((a, b) = params(q(x)))
    ) == :(
